...
var possibleCausesForAllergen: [String: Set<String>] = [:]

input
	.split(separator: "\n")
	.forEach { line in
		let pieces = line.components(separatedBy: " (contains ")
		let ingredients = pieces[0].components(separatedBy: " ")
		let allergens = pieces[1].dropLast().components(separatedBy: ", ")
		
		ingredients.forEach { countPerIngredient[$0, default: 0] += 1 }
		allergens.forEach { allergen in
			if let existingCauses = possibleCausesForAllergen[allergen] {
				possibleCausesForAllergen[allergen] = existingCauses.intersection(ingredients)
			} else {
				possibleCausesForAllergen[allergen] = Set(ingredients)
			}
		}
	}

var causes: [String: String] = [:]
while causes.count != possibleCausesForAllergen.count {
	possibleCausesForAllergen
		.filter { $0.value.count == 1 }
		.forEach { allergen, ingredients in
			let ingredient = ingredients.first!
			causes[allergen] = ingredient
			possibleCausesForAllergen.keys.forEach {
				possibleCausesForAllergen[$0]?.remove(ingredient)
			}
		}
}
...
func partOne() -> String {
	countPerIngredient
		.lazy
		.filter { causes.values.contains($0.key) == false }
		.map(\.value)
		.reduce(0, +)
		.description
}
