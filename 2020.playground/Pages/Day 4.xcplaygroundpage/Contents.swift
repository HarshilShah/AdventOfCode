...
func processPassport(_ raw: String) -> [String: String] {
	raw
		.components(separatedBy: .whitespacesAndNewlines)
		.reduce(into: [String: String]()) { dict, field in
			let key = String(field.prefix(while: { $0 != ":" }))
			let value = String(field.dropFirst(key.count + 1))
			dict[key] = value
		}
}
...
let validKeys: Set<String> = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]

func partOne() -> String {
	func isValid(_ passport: [String: String]) -> Bool {
		validKeys.allSatisfy(passport.keys.contains)
	}
...
func partTwo() -> String {
	let validEyeColours: Set<String> = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]

	func isValid(_ passport: [String: String]) -> Bool {
		guard validKeys.allSatisfy(passport.keys.contains) else { return false }

		guard let byr = Int(passport["byr"]!), (1920...2002).contains(byr) else { return false }
		guard let iyr = Int(passport["iyr"]!), (2010...2020).contains(iyr) else { return false }
		guard let eyr = Int(passport["eyr"]!), (2020...2030).contains(eyr) else { return false }

		let hgtTxt = passport["hgt"]!
		if hgtTxt.hasSuffix("cm"), let hgt = Int(hgtTxt.dropLast(2)), (150...193).contains(hgt) {}
		else if hgtTxt.hasSuffix("in"), let hgt = Int(hgtTxt.dropLast(2)), (59...76).contains(hgt) {}
		else { return false }

		guard let hcl = passport["hcl"], hcl.first == "#", hcl.dropFirst().count == 6, hcl.dropFirst().allSatisfy(\.isHexDigit) else { return false }
		guard let ecl = passport["ecl"], validEyeColours.contains(ecl) else { return false }
		guard let pid = passport["pid"], pid.count == 9, pid.allSatisfy(\.isNumber) else { return false }
